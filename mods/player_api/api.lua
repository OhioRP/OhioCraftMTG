...
PlayerAPI.registered_models = {}

-- Local for speed.
local models = PlayerAPI.registered_models
...
function PlayerAPI.register_model(name, def)
	models[name] = def
...
local players = {}
PlayerAPI.player_attached = {}

local function get_player_data(player)
	return assert(players[player:get_player_name()])
end

function PlayerAPI.get_animation(player)
	return get_player_data(player)
end

-- Called when a player's appearance needs to be updated
function PlayerAPI.set_model(player, model_name)
	...
	if model then
		...
		PlayerAPI.set_animation(player, "stand")
	else
		player:set_properties({
...
function PlayerAPI.set_texture(player, index, texture)
	---@diagnostic disable-next-line: undefined-field
	local textures = table.copy(PlayerAPI.get_textures(player))
	textures[index] = texture
	PlayerAPI.set_textures(player, textures)
end

function PlayerAPI.set_animation(player, anim_name, speed)
	local player_data = get_player_data(player)
	local model = models[player_data.model]
...
minetest.register_on_joinplayer(function(player)
	...
	players[name] = {}
	PlayerAPI.player_attached[name] = false
end)

minetest.register_on_leaveplayer(function(player)
	local name = player:get_player_name()
	players[name] = nil
	PlayerAPI.player_attached[name] = nil
end)

-- Localize for better performance.
local player_set_animation = PlayerAPI.set_animation
local player_attached = PlayerAPI.player_attached
...
minetest.register_globalstep(function(...)
	...
	PlayerAPI.globalstep(...)
end)

for _, api_function in pairs({ "get_animation", "set_animation", "set_model", "set_textures" }) do
	local original_function = PlayerAPI[api_function]
	PlayerAPI[api_function] = function(player, ...)
