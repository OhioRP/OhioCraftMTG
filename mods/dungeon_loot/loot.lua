...
function DungeonLoot.register(t)
	if t.name ~= nil then
		t = { t } -- single entry
	end
	for _, loot in ipairs(t) do
		table.insert(DungeonLoot.registered_loot, loot)
	end
end

function DungeonLoot._internal_get_loot(pos_y, dungeontype)
	-- filter by y pos and type
	local ret = {}
	for _, l in ipairs(DungeonLoot.registered_loot) do
